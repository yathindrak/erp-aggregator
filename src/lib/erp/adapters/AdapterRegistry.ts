...
export class AdapterRegistry {
    constructor(private factories: Record<string, () => IErpAdapterPlugin<any>> = ADAPTER_FACTORIES) { }
    ...
    create(erpName: string): IErpAdapterPlugin<any> {
        ...
        if (!factory) {
            throw new Error(`Unsupported ERP: "${erpName}". Available: ${Object.keys(this.factories).join(', ')}`);
        }
        return factory();
    }
